...
class AppSyncClient {
  ...
  async getCardIdsByAudioTaskId(task_id) {
    var query = gql(`
      query GetCardIdsByAudioTaskId($task_id: String!) {
        getCardIdsByAudioTaskId(audio_task_id: $task_id) {
          items {
            card_id
            audio_task_id
          }
    ...
    return response.data.getCardIdsByAudioTaskId.items; 
  }

  async createCard(cardInputVariables) {
    var mutation = gql(`
    ...
    `);
    var variables = { createCardInput: cardInputVariables };
    var response = await this.client.mutate({
      mutation: mutation,
      variables: variables
    });
  ...
  async updateCard(cardInputVariables) {
    var mutation = gql(`
    ...
    `);
    var variables = { updateCardInput: cardInputVariables };
    var response = await this.client.mutate({
      mutation: mutation,
      variables: variables
    });
  ...
  async query(graphql, variables, fetch_policy = 'network-only') {
    var query = gql(graphql);
    const client = await this.client.hydrated();
    var response = this.client.query({
      query: query, 
      variables: variables,
